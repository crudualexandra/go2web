...
func performPlainHTTPGet(host: String, port: Int, path: String) -> Int32 {
    ...
    while true {
        ...
        let request = "GET \(current.path.isEmpty ? "/" : current.path) HTTP/1.1\r\nHost: \(hostHeader)\r\nUser-Agent: go2web-swift/1.0\r\nAccept: text/html, application/json\r\nConnection: close\r\n\r\n"
        ...
        // Close the socket before processing redirects/response
        close(fd)

        let response = String(decoding: responseBytes, as: UTF8.self)

        func handleHeadAndBody(head: String, body: String) -> Int32 {
            ...
            if let code = statusCode, [301, 302, 303, 307, 308].contains(code), let loc = locationHeader {
                ...
                return -1 // signal to follow redirect
            }
        ...
        if let sep = response.range(of: "\r\n\r\n") {
            ...
            let result = handleHeadAndBody(head: head, body: body)
            if result == -1 { continue } else { return result }
